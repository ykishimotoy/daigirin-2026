...
今回BLEの活用方法を中心とした構成となっているため、具体的な技術説明をかなり飛ばしながら進めていきます。BLE の通信プロトコルやビーコンの仕組みについて詳しく知りたい方は、堤修一・松村礼央著『iOS×BLE Core Bluetooth プログラミング』（ソシム、2015 年）を参照してください。BLE の規格から iPhone での実際の送受信の仕方までわかりやすく書かれた本です。本章では仕組みの解説は最小限にとどめ、Android で iBeacon の送受信を実装する方法に絞って解説します。
...
しかし、BLE自体の技術は進歩しており、今後どこかで大きな変換点が来ると私は革新しています。
...
![接触確認アプリ「COCOA」の概要](./images_harutiro/cocoa.png)
...
BLEビーコンの電波情報を用いたFP（Finger Print）方式の屋内測位は、電波の強度や到達時間をもとに位置を推定する方法で、数メートル単位の精度が期待できます。GPS が届かない環境でのナビゲーションや位置情報サービスに活用されています。この仕組みはWi-Fiを用いた屋内測位にも応用されており、BLEビーコンと組み合わせることでさらに精度を高めることができます。
...
本章では 実際にAndroidからBLEビーコンを発信と受信をして電波が取得できるか検証をしてみたいと思います。
...
例えば、家のSSIDの情報を知っていると家のSSIDの電波を検出したときに家にいると推定できます。
